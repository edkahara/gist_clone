defmodule GistClone.Gists do
  ...
  def create_gist(user, attrs \\ %{}) do
    user
    |> Ecto.build_assoc(:gists) # Automatically add logged in user's id to the new gist
  ...
  def create_saved_gist(user, attrs \\ %{}) do
    user
    |> Ecto.build_assoc(:saved_gists) # Automatically add logged in user's id to the new gist
    |> Repo.insert()
  end
